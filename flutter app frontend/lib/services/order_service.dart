...
static Future<List<dynamic>> getMyAvailableOrders() async {
  ...
  final response = await http.get(
    Uri.parse('${app_api.Api.baseUrl}/orders/available'), // backend needs this
    headers: {
      'Authorization': 'Bearer $token',
    },
  );
  ...
  static Future<List<dynamic>> getMyAvailableAndAssignedOrders() async {
    final token = await AuthService.getToken();
    final response = await http.get(
      Uri.parse('${app_api.Api.baseUrl}/orders'), // You can use a general endpoint or create a custom one
      headers: {
        'Authorization': 'Bearer $token',
      },
    );
  ...
  final response = await http.get(
    Uri.parse('${app_api.Api.baseUrl}/orders/assigned'), // backend route you'll set up
    headers: {
      'Authorization': 'Bearer $token',
    },
  );
